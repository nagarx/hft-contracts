"""
Provenance capture for experiment lineage (pipeline-wide contract plane).

Phase 6 6B.4 (2026-04-17): Moved from `hft_ops.provenance.lineage` to
`hft_contracts.provenance` so the `Provenance` dataclass — which is embedded
in `ExperimentRecord` (6B.1 moves ExperimentRecord here too) — lives alongside
`canonical_hash`. This keeps the contract-plane self-contained: trainer code
can instantiate Provenance without depending on hft-ops, and hft-contracts
remains the import-free authoritative home for all cross-module data shapes.
...
I/O notes: this module uses stdlib `subprocess` (for git state capture) and
filesystem reads (for file / directory hashing). These are inherent to
provenance — capturing what produced an artifact requires reading the
producer's state. The I/O is **lazy**: import has no side effects, and
callers invoke the capture functions explicitly.
    ...
    *,
    ...
    Args:
        ...
        trainer_config_dict: When the manifest uses inline `trainer_config:`
            ...
            (when `Provenance` lives in hft_contracts, callers cannot post-
