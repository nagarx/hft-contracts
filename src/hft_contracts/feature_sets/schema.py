...
- ``hft_contracts.feature_sets.hashing`` — the canonical-form hash spec.
- Plan §4 + 6-agent validation (2026-04-15).

Phase 6 6B.3 (2026-04-17): Moved from ``hft_ops.feature_sets.schema`` to
``hft_contracts.feature_sets.schema`` so the FeatureSet contract lives
alongside ``canonical_hash`` + ``Provenance`` + ``SignalManifest`` on the
hft-contracts plane. Three consumers (evaluator, trainer, hft-ops) all
pin to this schema; single authoritative home prevents drift.
"""
